...
- `TOPICS`: A `.txt` file with all areas of interest, each in a separate line. 
...
```
#### Run
There is a list of existing areas in VIDURA, the new ones provided in the `TOPICS` input file will be appended to the list. To avoid duplications, run the following command to calculate the similarity between each new expertise and existing ones:
```bash
./similar_pairs.sh
```
